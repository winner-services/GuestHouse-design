...
function SupplierForm({ hideForm, singleSupplier }) {
    ...
    const submitData = async (e) => {
        ...
        let url = `createSupplier`
        let method = 'POST'
        if (form.id) {
            url = `updateSupplier/${form.id}`
            method = 'PUT'
    ...
    useEffect(() => {
        if (Object.values(singleSupplier).length > 0) {
            console.log(singleSupplier)
            for (const key in singleSupplier) {
                if (Object.hasOwnProperty.call(form, key)) {
                    const element = singleSupplier[key];
                    form[key] = element;
                    setForm({ ...form, [key]: element })
                }
            }
            setForm({ ...form, id: singleSupplier.id })
        }
    }, [])
